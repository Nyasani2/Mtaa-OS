// app/(os)/command/asis-simulator.tsx
// MTAA Command Centre — ASIS AI Simulator
// System audit, health checks, diagnostics
...
const SCENARIOS: SimulationScenario[] = [
  { id: 'full-audit', name: 'Full System Audit', description: 'Complete health check of all modules', icon: 'shield-alt', checks: ['auth', 'wallet', 'routes', 'database', 'api', 'storage'] },
  { id: 'auth-check', name: 'Auth Security Check', description: 'Verify authentication & authorization', icon: 'lock', checks: ['auth'] },
  { id: 'wallet-audit', name: 'Wallet Integrity', description: 'Check wallet functions & transactions', icon: 'wallet', checks: ['wallet'] },
  { id: 'route-scan', name: 'Route Scanner', description: 'Verify all app routes are working', icon: 'route', checks: ['routes'] },
  { id: 'pulse-health', name: 'Pulse Health', description: 'Check Pulse feed & media', icon: 'newspaper', checks: ['pulse'] },
  { id: 'db-integrity', name: 'Database Check', description: 'Verify tables, RLS, functions', icon: 'database', checks: ['database'] },
];

const ALL_CHECKS: AuditCheck[] = [
  { id: 'auth-1', category: 'Auth', name: 'Auth store loads', status: 'passed', message: 'Store initialized correctly', duration: 12 },
  { id: 'auth-2', category: 'Auth', name: 'User has id/email', status: 'failed', message: 'User object missing email field', duration: 8, details: 'Profile table needs email column' },
  { id: 'auth-3', category: 'Auth', name: 'PIN check works', status: 'passed', message: 'PIN validation functional', duration: 45 },
  { id: 'auth-4', category: 'Auth', name: 'Profile exposed', status: 'failed', message: 'Profile data not available', duration: 15, details: 'Check profile service connection' },
  { id: 'wallet-1', category: 'Wallet', name: 'Balance is number', status: 'passed', message: 'Balance type correct', duration: 5 },
  { id: 'wallet-2', category: 'Wallet', name: 'Balance >= 0', status: 'passed', message: 'Balance non-negative', duration: 3 },
  { id: 'wallet-3', category: 'Wallet', name: 'Transactions array', status: 'passed', message: 'Transaction history available', duration: 120 },
  { id: 'wallet-4', category: 'Wallet', name: 'Send function exists', status: 'failed', message: 'send() not found in store', duration: 8, details: 'Add send method to wallet store' },
  { id: 'wallet-5', category: 'Wallet', name: 'Receive function exists', status: 'failed', message: 'receive() not found in store', duration: 6, details: 'Add receive method to wallet store' },
  ...
  { id: 'routes-1', category: 'Routes', name: 'Route /(os)/wallet', status: 'passed', message: 'Wallet route exists', duration: 20 },
  { id: 'routes-2', category: 'Routes', name: 'Route /(os)/wallet/deposit', status: 'passed', message: 'Deposit route exists', duration: 15 },
  { id: 'routes-3', category: 'Routes', name: 'Route /(os)/wallet/withdraw', status: 'passed', message: 'Withdraw route exists', duration: 15 },
  { id: 'routes-4', category: 'Routes', name: 'Route /(os)/wallet/send', status: 'passed', message: 'Send route exists', duration: 15 },
  { id: 'routes-5', category: 'Routes', name: 'Route /(os)/wallet/qr', status: 'passed', message: 'QR route exists', duration: 15 },
  { id: 'routes-6', category: 'Routes', name: 'Route /(os)/clock', status: 'passed', message: 'Clock route exists', duration: 10 },
  { id: 'routes-7', category: 'Routes', name: 'Route /(os)/calculator', status: 'passed', message: 'Calculator route exists', duration: 10 },
  { id: 'routes-8', category: 'Routes', name: 'Route /(os)/calendar', status: 'passed', message: 'Calendar route exists', duration: 10 },
  { id: 'routes-9', category: 'Routes', name: 'Route /(os)/network', status: 'passed', message: 'Network route exists', duration: 10 },
  { id: 'routes-10', category: 'Routes', name: 'Route /(os)/wifi', status: 'passed', message: 'WiFi route exists', duration: 10 },
  { id: 'pulse-1', category: 'Pulse', name: 'Pulse events table', status: 'passed', message: 'pulse_events exists', duration: 25 },
  { id: 'pulse-2', category: 'Pulse', name: 'Resolved media URLs', status: 'passed', message: 'Media URL resolution working', duration: 30 },
  ...
  { id: 'pulse-4', category: 'Pulse', name: 'Feed engine', status: 'passed', message: 'Feed queries functional', duration: 40 },
  { id: 'db-1', category: 'Database', name: 'Supabase connection', status: 'passed', message: 'Connected to exmfzrgsxnwwliatva', duration: 50 },
  ...
  { id: 'db-3', category: 'Database', name: 'Edge functions', status: 'passed', message: '12 functions deployed', duration: 100 },
  { id: 'api-1', category: 'API', name: 'Wallet operations', status: 'passed', message: 'wallet-operations function active', duration: 80 },
  { id: 'api-2', category: 'API', name: 'Checkout', status: 'passed', message: 'checkout function active', duration: 60 },
  { id: 'api-3', category: 'API', name: 'Notifications', status: 'passed', message: 'notification-action function active', duration: 55 },
  { id: 'storage-1', category: 'Storage', name: 'Content bucket', status: 'passed', message: 'content bucket accessible', duration: 30 },
  { id: 'storage-2', category: 'Storage', name: 'Avatar bucket', status: 'passed', message: 'avatars bucket accessible', duration: 25 },
];
      ...
      <View style={styles.gaugeContainer}>
        ...
        <View style={styles.gaugeStats}>
          <View style={styles.statItem}>
            ...
            <Text style={styles.statText}>{result.passed} Passed</Text>
          </View>
          <View style={styles.statItem}>
            <Ionicons name="close-circle" size={16} color="#ff453a" />
            <Text style={styles.statText}>{result.failed} Failed</Text>
          </View>
          <View style={styles.statItem}>
            <Ionicons name="warning" size={16} color="#ff9f0a" />
            <Text style={styles.statText}>{result.warnings} Warnings</Text>
          </View>
        </View>
      </View>
    );
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => setShowScenarioModal(true)} style={styles.headerBtn}>
      ...
      <View style={styles.scenarioBar}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {SCENARIOS.map((s) => (
            <TouchableOpacity
              key={s.id}
              style={[styles.scenarioChip, selectedScenario === s.id && styles.scenarioChipActive]}
      ...
      <TouchableOpacity
        ...
        onPress={() => runAudit(selectedScenario)}
        disabled={isRunning}
      >
        {isRunning ? (
          <>
            <ActivityIndicator size="small" color="#fff" />
            <Text style={styles.runButtonText}>Running Audit...</Text>
          </>
        ) : (
          <>
            <Ionicons name="play" size={20} color="#fff" />
            <Text style={styles.runButtonText}>Run {SCENARIOS.find((s) => s.id === selectedScenario)?.name}</Text>
          </>
        )}
      </TouchableOpacity>
      ...
      <ScrollView style={styles.content}>
          ...
          <>
              ...
              <View style={styles.checksList}>
                {result.checks.map((check) => (
                  <TouchableOpacity
                    ...
                    onPress={() => setExpandedCheck(expandedCheck === check.id ? null : check.id)}
                  >
                      ...
                      <View style={styles.checkDetails}>
                        ...
                        {check.status === 'failed' && (
                          <View style={styles.fixBox}>
                            ...
                            <Text style={styles.fixText}>{getFixSuggestion(check)}</Text>
                          </View>
                        )}
                      </View>
                    )}
            ...
            {!result && !isRunning && (
              <View style={styles.emptyAudit}>
                ...
                <Text style={styles.emptyTitle}>Ready to Audit</Text>
                <Text style={styles.emptySubtitle}>Select a scenario and run the audit to check system health.</Text>
              </View>
            )}
          </>
        )}
        ...
        {activeTab === 'history' && (
          <View style={styles.historyList}>
            {history.map((h, i) => (
              <TouchableOpacity key={i} style={styles.historyItem} onPress={() => setResult(h)}>
                <View style={styles.historyHeader}>
                  ...
                  <Text style={[styles.historyScore, { color: h.failed === 0 ? '#30d158' : h.failed > 2 ? '#ff453a' : '#ff9f0a' }]}>
                    {h.passed}/{h.total}
                  </Text>
                </View>
            ...
            {history.length === 0 && (
              <Text style={styles.emptyHistory}>No audit history yet. Run your first audit!</Text>
            )}
          </View>
        )}
            ...
            {ALL_CHECKS.filter((c) => c.status === 'failed').map((check) => (
              <View key={check.id} style={styles.fixItem}>
                <View style={styles.fixHeader}>
                  ...
                  <Text style={styles.fixItemTitle}>{check.name}</Text>
                </View>
                <Text style={styles.fixItemMessage}>{check.message}</Text>
                <Text style={styles.fixItemSolution}>{getFixSuggestion(check)}</Text>
              </View>
            ))}
            {ALL_CHECKS.filter((c) => c.status === 'failed').length === 0 && (
              <Text style={styles.allGood}>All checks passing! No fixes needed.</Text>
            )}
          </View>
        )}
      </ScrollView>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {SCENARIOS.map((s) => (
              <TouchableOpacity
                key={s.id}
                style={[styles.modalItem, selectedScenario === s.id && styles.modalItemActive]}
                onPress={() => { setSelectedScenario(s.id); setShowScenarioModal(false); }}
              >
                ...
                <View style={styles.modalItemInfo}>
                  ...
                  <Text style={styles.modalItemDesc}>{s.description}</Text>
                </View>
              </TouchableOpacity>
            ))}
          </View>
