...
class PermissionSystem {
  ...
  async requestPermission(appId: string, permission: Permission): Promise<boolean> {
    ...
    const existing = appGrants.find(g => g.permission === permission);
  ...
  hasPermission(appId: string, permission: Permission): boolean {
    return (this.grants.get(appId) || []).some(g => g.permission === permission && g.granted);
  }
  ...
  revokePermission(appId: string, permission: Permission): void {
    const grant = (this.grants.get(appId) || []).find(g => g.permission === permission);
