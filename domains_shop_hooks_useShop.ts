...
export function useShop(shopId?: string) {
  ...
  }, [shopId, fetchShop, fetchProducts, fetchOrders, fetchCustomers, fetchAnalytics]);
...
// Standalone hooks for individual features
export function useShopOrders(shopId?: string) {
  const { orders, fetchOrders, loading, error, updateOrderStatus } = useShop(shopId);
  return { orders, fetchOrders, loading, error, updateOrderStatus };
}

export function useShopProducts(shopId?: string) {
  const { products, fetchProducts, loading, error, addProduct, updateProduct, deleteProduct } = useShop(shopId);
  return { products, fetchProducts, loading, error, addProduct, updateProduct, deleteProduct };
}
