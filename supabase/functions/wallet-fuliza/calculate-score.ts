...
serve(async (req) => {
  ...
  try {
    ...
    const walletAgeScore = Math.min(walletAgeDays * 2, 200) // Max 200 for 100+ days
    const txVolumeScore = Math.min(txVolume30d / 1000, 250) // Max 250 for KES 250k
    const txCountScore = Math.min(txCount30d * 10, 150) // Max 150 for 15+ tx
    const savingsScore = Math.min(savingsBalance / 500, 100) // Max 100 for KES 50k
    const escrowScore = escrowRate * 100
    const baseScore = 200 // Base score
    ...
    const suggestedLimit = Math.min(creditScore * 100, 50000) // Max KES 50k
