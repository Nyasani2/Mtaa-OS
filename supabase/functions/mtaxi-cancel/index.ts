...
serve(async (req) => {
  ...
  const { data: ride } = await supabase.from("mtaxi_rides").select("*").eq("id", ride_id).single();
  ...
  const { error: updateError } = await supabase.from("mtaxi_rides").update({ status: "cancelled", cancelled_at: new Date().toISOString(), cancellation_reason: reason }).eq("id", ride_id);
  if (updateError) return new Response(JSON.stringify({ error: updateError.message }), { status: 500 });

  try {
    await supabase.from("wallet_escrows").update({ status: "cancelled", updated_at: new Date().toISOString() }).eq("reference_id", ride_id).eq("status", "held");
  } catch (_e) {}
