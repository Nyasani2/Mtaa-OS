...
serve(async (req) => {
  ...
  try {
    ...
    if (transaction_type === 'payment' || transaction_type === 'transfer') {
      ...
      taxes.push({ tax_type: 'vat', tax_rate: vatRate, tax_amount: vatAmount, taxable_amount: amount })
      totalTax += vatAmount
    }
    ...
    taxes.push({ tax_type: 'excise', tax_rate: countryConfig.excise_duty_rate, tax_amount: exciseAmount, taxable_amount: feeAmount })
    ...
    if (business_wallet_id && amount >= countryConfig.withholding_tax_threshold) {
      ...
      taxes.push({ tax_type: 'withholding', tax_rate: countryConfig.withholding_tax_rate, tax_amount: whAmount, taxable_amount: amount })
      totalTax += whAmount
    }

    if (amount >= 100000) {
      const dstAmount = Math.round(amount * countryConfig.digital_service_tax_rate * 100) / 100
      taxes.push({ tax_type: 'digital_service_tax', tax_rate: countryConfig.digital_service_tax_rate, tax_amount: dstAmount, taxable_amount: amount })
      totalTax += dstAmount
    }
