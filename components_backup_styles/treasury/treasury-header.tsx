...
import { Bell, User, Globe, Landmark } from "lucide-react-native";
  ...
  useEffect(() => {
    async function load() {
      ...
      if (user) {
        ...
        const { count } = await supabase.from("treasury_expenditures").select("*", { count: "exact", head: true }).eq("country_code", countryCode).eq("payment_status", "pending");
        setNotifications(count || 0);
      }
      const { data: cfg } = await supabase.from("treasury_country_config").select("*").eq("country_code", countryCode).single();
      setConfig(cfg);
    }
    load();
  }, []);
  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6">
      <div className="flex items-center gap-4">
        ...
        <h2 className="text-lg font-semibold text-slate-900">{config?.treasury_name || "National Treasury"}</h2>
        <Badge variant="outline" className="flex items-center gap-1"><Globe className="w-3 h-3" />{config?.country_name || countryCode}</Badge>
        <Badge variant="secondary">FY {config?.current_fiscal_year} Q{config?.current_quarter}</Badge>
      </div>
