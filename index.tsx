...
import { detectUserEducationRole } from '@/domains/education/services/education-role-guard';
...
export default function EducationLanding() {
  ...
  const [detectingRole, setDetectingRole] = useState(true);
  const [userRole, setUserRole] = useState<string | null>(null);

  useEffect(() => {
    if (!user?.id) { setDetectingRole(false); loadData(); return; }
  ...
  const roleDisplayName = (role: string | null) => {
    if (!role) return 'My Portal';
    return role.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()) + ' Portal';
  };
  ...
  if (detectingRole) {
    return (
      <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
        ...
        <Text style={{ color: '#94a3b8', marginTop: 12 }}>Detecting your education profile...</Text>
      </View>
    );
  }
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        >
          ...
          <View style={{ flex: 1, marginLeft: 12 }}>
            <Text style={styles.portalTitle}>{roleDisplayName(userRole)}</Text>
            <Text style={styles.portalSub}>
              {userRole
                ? 'Tap to open your personalized role-based dashboard'
                : 'Not enrolled yet. Tap to register or browse schools.'}
            </Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  portalBanner: { flexDirection: 'row', alignItems: 'center', marginHorizontal: 16, marginBottom: 12, padding: 14, borderRadius: 12 },
  portalTitle: { fontSize: 15, fontWeight: '700', color: '#fff' },
  portalSub: { fontSize: 12, color: 'rgba(255,255,255,0.8)', marginTop: 2 },
