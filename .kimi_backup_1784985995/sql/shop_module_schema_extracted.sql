-- ============================================================
-- MTAA AFRIQ SHOP MODULE - Complete SQL Schema
-- Business Operating System: POS, Inventory, Accounting, Affiliate, Escrow
-- ============================================================
...
CREATE TABLE IF NOT EXISTS shops (
    ...
    description TEXT,
    category TEXT NOT NULL, -- retail, restaurant, bar, pharmacy, etc.
    ...
    tax_number TEXT,
    business_type TEXT, -- sole_prop, pty_ltd, partnership, cooperative
...
CREATE TABLE IF NOT EXISTS shop_products (
    ...
    -- Variants (JSONB for size, color, etc.)
...
CREATE TABLE IF NOT EXISTS purchase_orders (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    shop_id UUID NOT NULL REFERENCES shops(id) ON DELETE CASCADE,
    supplier_id UUID, -- Could link to suppliers table
    supplier_name TEXT,
...
CREATE TABLE IF NOT EXISTS shop_orders (
    ...
    payment_method TEXT, -- cash, card, wallet, escrow
    ...
    -- Affiliate
    affiliate_id UUID, -- References shop_affiliates
...
CREATE TABLE IF NOT EXISTS shop_accounts (
    ...
    sub_type TEXT, -- cash, inventory, accounts_receivable, sales, cost_of_goods, rent, etc.
...
CREATE TABLE IF NOT EXISTS shop_expenses (
    ...
    category TEXT NOT NULL, -- rent, utilities, salaries, supplies, marketing, etc.
    description TEXT,
...
-- Affiliate Program Settings (per shop)
CREATE TABLE IF NOT EXISTS shop_affiliate_programs (
    ...
    commission_value NUMERIC(12,2) DEFAULT 10, -- Percentage or fixed amount
...
-- Products listed in marketplace (synced from shop_products)
CREATE TABLE IF NOT EXISTS marketplace_listings (
    ...
    priority_score INTEGER DEFAULT 0, -- For search ranking
...
-- Customer Messages (shop-customer chat)
...
-- Shops: Owners can manage their shops, public can view active shops
CREATE POLICY "Shops owner full access" ON shops
    FOR ALL USING (owner_id = auth.uid());
CREATE POLICY "Shops public view" ON shops
    FOR SELECT USING (status = 'active');

-- Shop Staff: Staff can access their shop
CREATE POLICY "Shop staff access" ON shop_staff
    FOR ALL USING (
        shop_id IN (SELECT id FROM shops WHERE owner_id = auth.uid())
        OR user_id = auth.uid()
    );

-- Products: Shop owners/staff can manage, public can view active
CREATE POLICY "Products shop manage" ON shop_products
    FOR ALL USING (
        shop_id IN (SELECT id FROM shops WHERE owner_id = auth.uid())
    );
CREATE POLICY "Products public view" ON shop_products
    FOR SELECT USING (
        shop_id IN (SELECT id FROM shops WHERE status = 'active')
        AND is_active = true
    );

-- Orders: Shop owners/staff can manage, customers can view their own
CREATE POLICY "Orders shop manage" ON shop_orders
    FOR ALL USING (
        shop_id IN (SELECT id FROM shops WHERE owner_id = auth.uid())
    );
CREATE POLICY "Orders customer view" ON shop_orders
    FOR SELECT USING (customer_id = auth.uid());

-- Messages: Participants can view
CREATE POLICY "Messages participants" ON shop_messages
    FOR ALL USING (
...
CREATE POLICY "Affiliates user view" ON shop_affiliates
    FOR ALL USING (user_id = auth.uid());

-- Reviews: Public can view, customers can create/update their own
CREATE POLICY "Reviews public view" ON shop_reviews
    FOR SELECT USING (is_visible = true);
CREATE POLICY "Reviews customer manage" ON shop_reviews
...
-- Update shop metrics on order completion
...
CREATE TRIGGER trigger_update_shop_metrics
    AFTER UPDATE ON shop_orders
    FOR EACH ROW
    EXECUTE FUNCTION update_shop_metrics();

-- Auto-create inventory transaction on order
CREATE OR REPLACE FUNCTION create_inventory_sale_transaction()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO inventory_transactions (
        shop_id, product_id, type, quantity, unit_cost, total_cost,
    ...
    WHERE o.id = NEW.id AND NEW.status = 'delivered' AND OLD.status != 'delivered';
...
CREATE TRIGGER trigger_inventory_sale
    ...
    EXECUTE FUNCTION create_inventory_sale_transaction();

-- Update product stock on inventory transaction
...
CREATE TRIGGER update_marketplace_listings_updated_at BEFORE UPDATE ON marketplace_listings
    FOR EACH ROW EXECUTE FUNCTION update_updated_at();
CREATE TRIGGER update_shop_affiliates_updated_at BEFORE UPDATE ON shop_affiliates
    FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
FROM (
    ...
    ('4100', 'Shipping Revenue', 'revenue', 'shipping'),
    ('5000', 'Cost of Goods Sold', 'expense', 'cogs'),
    ('6000', 'Rent Expense', 'expense', 'rent'),
    ('6100', 'Utilities Expense', 'expense', 'utilities'),
    ('6200', 'Salaries Expense', 'expense', 'salaries'),
    ('6300', 'Marketing Expense', 'expense', 'marketing')
) AS defaults(code, name, type, sub_type)
CROSS JOIN shops;
...
CREATE OR REPLACE FUNCTION get_shop_dashboard(shop_uuid UUID)
RETURNS JSONB AS $$
DECLARE
    result JSONB;
BEGIN
    SELECT jsonb_build_object(
        ...
        'low_stock_items', (
            ...
            WHERE shop_id = shop_uuid AND stock_quantity <= stock_alert_level
        ),
        'today_sales', COALESCE(SUM(total_amount) FILTER (WHERE created_at >= CURRENT_DATE), 0),
        'today_orders', COUNT(*) FILTER (WHERE created_at >= CURRENT_DATE)
    )
    INTO result
    FROM shop_orders
    WHERE shop_id = shop_uuid AND status = 'delivered';
...
RETURNS NUMERIC AS $$
DECLARE
    program shop_affiliate_programs%ROWTYPE;
    commission NUMERIC;
BEGIN
    SELECT * INTO program
    FROM shop_affiliate_programs
    WHERE shop_id = (SELECT shop_id FROM shop_affiliates WHERE id = p_affiliate_id);

    IF program.commission_type = 'percentage' THEN
        commission := p_order_amount * (program.commission_value / 100);
    ELSIF program.commission_type = 'fixed' THEN
        commission := program.commission_value;
    ELSIF program.commission_type = 'tiered' THEN
        -- Simplified tier logic - can be expanded
        commission := p_order_amount * (program.commission_value / 100);
    END IF;

    RETURN COALESCE(commission, 0);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
