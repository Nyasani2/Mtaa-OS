...
import { getPostsByHashtag } from '@/lib/services/streets-service';
...
export default function HashtagScreen() {
  ...
  const loadPosts = useCallback(async () => {
    ...
    try {
      const data = await getPostsByHashtag(decodedTag);
      setPosts(data);
    } catch (err: any) {
      ...
      >
        {item.media_url && item.media_type !== 'text' ? (
          ...
          <View style={[styles.gridImage, styles.textGridItem]}>
            <Text style={styles.textGridContent} numberOfLines={4}>
