// MTAA PROPERTY OS — BOOKING SERVICE
...
export class BookingService {
  async createBooking(booking: Partial<PropertyBooking>): Promise<PropertyBooking> {
  ...
  async getBookingById(id: string): Promise<PropertyBooking | null> {
    const { data, error } = await supabase
      .from("property_bookings")
  ...
  async getGuestBookings(guestId: string): Promise<PropertyBooking[]> {
    const { data, error } = await supabase
      .from("property_bookings")
  ...
  async getHostBookings(hostId: string): Promise<PropertyBooking[]> {
    const { data, error } = await supabase
      .from("property_bookings")
