...
import type { StayListing, StayBooking, Lease, MaintenanceTicket, StayPayment } from "../types";
...
export class StayController {
  ...
  private async getLandlordDashboard(landlordId: string) {
    const [listings, leases, tickets, payments] = await Promise.all([
      stayService.getListings().then((p) => p.filter((prop) => prop.owner_id === landlordId)),
    ...
    return {
      ...
      monthlyRevenue,
      openTickets: tickets.filter((t) => !["closed", "paid"].includes(t.status)).length,
      vacancies: activeListings.length - activeLeases.length,
    };
  }
  ...
  private async getHostDashboard(hostId: string) {
    const [bookings, listings] = await Promise.all([
      ...
      stayService.getListings().then((p) => p.filter((prop) => prop.owner_id === hostId)),
    ]);
    ...
    return {
      ...
      totalRevenue,
      occupancyRate: listings.length > 0 ? (confirmedBookings.length / listings.length) * 100 : 0,
    };
  }
...
// Legacy alias
export const propertyController = stayController;
