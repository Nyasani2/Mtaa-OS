...
export default function LockScreen() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (valid) {
      setPin('');
      setError('');
      // Unlock successful — go to OS home
      router.replace('/(os)');
    } else {
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDark ? '#0a0a0f' : '#f8f9fa' }]}>
      ...
      <Text style={[styles.subtitle, { color: isDark ? '#9ca3af' : '#6b7280' }]}>
        {isLocked
          ? `Locked. Try again in ${Math.ceil(lockoutTimer / 60)}:${(lockoutTimer % 60).toString().padStart(2, '0')}`
          : 'Unlock your device'}
      </Text>
      ...
      {pin.length >= 4 && pin.length < MAX_PIN_LENGTH && (
        <TouchableOpacity style={styles.button} onPress={handleSubmit} disabled={isLocked}>
