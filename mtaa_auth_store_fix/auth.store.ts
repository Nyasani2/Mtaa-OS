...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      initialize: async () => {
        // ─── CRITICAL FIX: Force-clear stuck initPromise ──────────
        // If a previous init hung (component unmounted, storage locked, etc.)
        // initPromise would be stuck forever. We detect this by checking
        // if initialized is already true (shouldn't re-init) or if
        // initPromise exists but the store isn't initialized after a reasonable time.
        const state = get();
        if (state.initialized) return;

        // If initPromise exists from a previous session, clear it — it's stale
        if (initPromise) {
          console.warn('[Auth] Clearing stale initPromise from previous session');
          initPromise = null;
        }
        ...
        initPromise = (async () => {
          set({ isLoading: true });
          try {
            // ─── Timeout guard: getSession has 10s max ─────────────
