...
ok() { echo "[$(date '+%H:%M:%S')] ✅ $1"; MOVED_COUNT=$((MOVED_COUNT + 1)); }
...
# Create structure
mkdir -p "$BACKUP_ROOT"/route_groups
mkdir -p "$BACKUP_ROOT"/frontend_backups
mkdir -p "$BACKUP_ROOT"/lib_backups
mkdir -p "$BACKUP_ROOT"/zip_archives
mkdir -p "$BACKUP_ROOT"/orphaned_screens
mkdir -p "$BACKUP_ROOT"/old_asis
mkdir -p "$BACKUP_ROOT"/android_build
mkdir -p "$BACKUP_ROOT"/ios_build
mkdir -p "$BACKUP_ROOT"/misc
...
PROFILE_BACKUP="app/(os)/profile/.backup_v2"
if [ -d "$PROFILE_BACKUP" ]; then
    mv "$PROFILE_BACKUP" "$BACKUP_ROOT"/frontend_backups/profile_backup_v2_${TIMESTAMP}/
...
if [ "$ZIP_COUNT" -gt 0 ]; then
    ...
    for pattern in asis_*.zip garage_*.zip '*_fix*.zip' '*_complete*.zip'; do
        for f in $pattern; do
            if [ -f "$f" ]; then
                mv "$f" "$BACKUP_ROOT"/zip_archives/ 2>/dev/null || true
            fi
        done
    done
...
if [ -d "domains/regulatory/components" ]; then
    find domains/regulatory/components -name "*CBKDashboard*" -type f 2>/dev/null | while read -r f; do
        if echo "$f" | grep -qE '\(1\)| '; then
...
{
echo "# MTAA OS V10 — Phase 0 Archive Manifest"
echo "## Date: $(date)"
echo "## Action: Moved duplicates, backups, and orphans to backup folder (NOT deleted)"
echo "## Backup Location: $BACKUP_ROOT"
echo ""
echo "## Summary"
echo "- Total items moved: $(find "$BACKUP_ROOT" -type f | wc -l)"
echo "- Total size: $(du -sh "$BACKUP_ROOT" | cut -f1)"
echo "- Warnings encountered: ${#WARNINGS[@]}"
echo ""
echo "## Warnings Requiring Manual Review"
if [ ${#WARNINGS[@]} -eq 0 ]; then
    echo "None — all items were exact duplicates or safe to move."
else
...
for dir in app/os app/regulatory "app/(wallet)" wallet settings; do
    if [ -e "$dir" ]; then
        ...
        REMAINING_DUPES=$((REMAINING_DUPES + 1))
    fi
done
...
for dir in lib/asis-v6 lib/asis-v7; do
    if [ -d "$dir" ]; then
        warn "$dir still exists"
        REMAINING_ASIS=$((REMAINING_ASIS + 1))
    fi
done
