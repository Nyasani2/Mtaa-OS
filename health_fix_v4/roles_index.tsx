...
export default function StaffManagementScreen() {
  ...
  // v4 FIX: System admins can view ALL staff globally
  ...
  const fetchStaff = useCallback(async () => {
    if (!canViewStaff) {
      setStaff([]);
      setStats({ active: 0, pending: 0, total: 0 });
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      let data: any[] = [];
      let staffStats = { active: 0, pending: 0, total: 0 };

      if (isSystemAdmin) {
        // v4: System admin sees ALL staff across ALL facilities
        data = await healthRoleService.getAllStaffForSystemAdmin(filters);
        staffStats = await healthRoleService.getStaffStats(null); // null = global stats
      } else if (facilityId) {
        // Facility admin sees only their facility's staff
        data = await healthRoleService.getStaffByFacility(facilityId, filters);
        staffStats = await healthRoleService.getStaffStats(facilityId);
      }
  ...
  const handleInvite = async () => {
    ...
    try {
      await healthRoleService.inviteStaff({
        ...
        facilityId: isSystemAdmin ? undefined : facilityId || undefined,
      });
      setShowInvite(false);
      setInviteEmail('');
      setInviteDept('');
  ...
  const handleSuspend = async (staffId: string) => {
    try {
      ...
      alert('Suspend failed: ' + err.message);
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        {isSystemAdmin && (
          <View style={styles.adminBadge}>
            ...
            <Text style={styles.adminBadgeText}>Global</Text>
          </View>
        )}
      </View>
      ...
      {isSystemAdmin && (
        <View style={styles.scopeBanner}>
          ...
          <Text style={styles.scopeText}>
            System Admin View — All Facilities
          </Text>
        </View>
      )}
      ...
      <View style={styles.statsRow}>
        <View style={[styles.statCard, { backgroundColor: '#dcfce7' }]}>
          ...
          <Text style={[styles.statNumber, { color: '#16a34a' }]}>{stats.active}</Text>
          <Text style={styles.statLabel}>Active</Text>
        </View>
        <View style={[styles.statCard, { backgroundColor: '#fef9c3' }]}>
          <AlertCircle size={20} color="#ca8a04" />
          <Text style={[styles.statNumber, { color: '#ca8a04' }]}>{stats.pending}</Text>
      ...
      <View style={styles.filterSection}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterRow}>
          ...
          >
            <Text style={[styles.filterChipText, roleFilter === 'All Roles' && styles.filterChipTextActive]}>
              All Roles
            </Text>
          </TouchableOpacity>
          {ROLE_FILTERS.map((r) => (
        ...
        {loading && !refreshing ? (
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySubtitle}>
              {canViewStaff
                ? 'Try adjusting your filters or invite new staff.'
                : 'No facility assigned. Contact a system administrator.'}
            </Text>
          </View>
        ) : (
          filteredStaff.map((s) => (
            <View key={s.id} style={styles.staffCard}>
              <View style={styles.staffHeader}>
                <View style={[styles.roleBadge, { backgroundColor: ROLE_COLORS[s.role as HealthRole] + '20' }]}>
                  <Text style={[styles.roleBadgeText, { color: ROLE_COLORS[s.role as HealthRole] }]}>
                    {ROLE_DISPLAY_NAMES[s.role as HealthRole] || s.role}
                  </Text>
                </View>
                <View style={[styles.statusBadge, { backgroundColor: s.status === 'active' ? '#dcfce7' : s.status === 'pending' ? '#fef9c3' : '#fee2e2' }]}>
                  <Text style={{ color: s.status === 'active' ? '#16a34a' : s.status === 'pending' ? '#ca8a04' : '#dc2626', fontSize: 11, fontWeight: '600', textTransform: 'capitalize' }}>
                    {s.status}
                  </Text>
                </View>
              </View>
              ...
              {s.user_email && s.user_email !== s.user_full_name && (
              ...
              {isSystemAdmin && s.facility_name && (
                <View style={styles.facilityRow}>
                  ...
                  <Text style={styles.facilityText}>{s.facility_name}</Text>
                </View>
              )}

              {s.department && (
                <Text style={styles.staffMeta}>Dept: {s.department}</Text>
              )}

              <View style={styles.staffActions}>
                {s.status === 'pending' && (
                  <TouchableOpacity style={[styles.actionBtn, { backgroundColor: '#dcfce7' }]} onPress={() => handleApprove(s.id)}>
                    <CheckCircle2 size={16} color="#16a34a" />
                    <Text style={[styles.actionBtnText, { color: '#16a34a' }]}>Approve</Text>
                  </TouchableOpacity>
                )}
                {s.status === 'active' && (
                  <TouchableOpacity style={[styles.actionBtn, { backgroundColor: '#fee2e2' }]} onPress={() => handleSuspend(s.id)}>
