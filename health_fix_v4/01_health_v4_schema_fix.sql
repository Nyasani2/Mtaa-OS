-- ============================================
-- MTAA Health OS v4: Schema + RPC + RLS Fix
...
-- =====================================================
-- 2. FIX RLS POLICIES (remove references to non-existent columns)
-- =====================================================

-- Drop old broken policies that reference hs.specialization
...
-- Recreate FIXED policies (no specialization references)
...
CREATE POLICY "Hospital admins view facility staff"
  ...
  USING (
    EXISTS (
      ...
      AND hs.status = 'active'
      AND hs.facility_id = health_staff.facility_id
    )
  );

CREATE POLICY "Staff manage own record"
  ON health_staff FOR ALL
  USING (user_id = auth.uid());
...
-- Fix health_facility_registrations policies
...
-- =====================================================
-- 3. CREATE/REPLACE RPC: health_get_primary_staff_record
-- SECURITY DEFINER to bypass RLS recursion
-- Returns the user's most recent active staff record
-- =====================================================
...
-- =====================================================
-- 4. CREATE/REPLACE RPC: health_get_all_staff_for_admin
-- For system admins to view ALL staff across ALL facilities
-- =====================================================
CREATE OR REPLACE FUNCTION health_get_all_staff_for_admin(
...
-- =====================================================
-- 5. CREATE/REPLACE RPC: health_get_staff_by_facility
-- For facility admins to view staff in their facility
-- =====================================================
...
-- =====================================================
-- 6. CREATE/REPLACE RPC: health_get_staff_stats
-- Returns count of active/pending/total staff
-- =====================================================
CREATE OR REPLACE FUNCTION health_get_staff_stats(
