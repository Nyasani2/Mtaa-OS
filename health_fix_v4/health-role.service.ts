...
export const ROLE_COLORS: Record<HealthRole, string> = {
  ...
  radiologist: '#ea580c',
  hospital_admin: '#ca8a04',
  cashier: '#db2777',
  hr_manager: '#4f46e5',
  accountant: '#059669',
  ambulance_driver: '#b91c1c',
  receptionist: '#6b7280',
};

// Helper to detect if a table is missing
...
class HealthRoleService {
  /**
   * v4: Uses RPC to get primary staff record.
  ...
  /**
   * v4: Get staff stats (active/pending/total)
   * For system_admin: pass null facility_id to get global stats
   * For facility admin: pass their facility_id
   */
  async getStaffStats(facilityId?: string | null): Promise<{ active: number; pending: number; total: number }> {
    try {
      const { data, error } = await supabase
        .rpc('health_get_staff_stats', {
          p_facility_id: facilityId || null,
        });

      if (error) {
        console.error('[HealthRoleService] getStaffStats error:', error);
      ...
      return {
        active: Number(data[0].active_count) || 0,
        pending: Number(data[0].pending_count) || 0,
        total: Number(data[0].total_count) || 0,
      };
    } catch (err: any) {
      console.error('[HealthRoleService] getStaffStats exception:', err);
      return { active: 0, pending: 0, total: 0 };
    }
  }

  /**
   * v4: Get ALL staff across ALL facilities (for system_admin)
   */
  async getAllStaffForSystemAdmin(filters?: {
    status?: string;
    role?: string;
    search?: string;
    ...
    try {
      const { data, error } = await supabase
        .rpc('health_get_all_staff_for_admin', {
  ...
  /**
   * v4: Get staff for a specific facility (for facility admins)
   */
  ...
  async approveStaff(staffId: string): Promise<void> {
    const { error } = await supabase
      .from('health_staff')
      .update({ status: 'active', onboarding_status: 'in_progress' })
      .eq('id', staffId);
