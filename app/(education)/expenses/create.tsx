...
export default function CreateExpenseScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("education_expenses").insert({
        title: title.trim(),
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.form}>
        ...
        <TextInput style={styles.input} value={title} onChangeText={setTitle} placeholder="e.g. Stationery Purchase" placeholderTextColor="#64748b" />
        ...
        <TextInput style={styles.input} value={category} onChangeText={setCategory} placeholder="e.g. Supplies, Maintenance" placeholderTextColor="#64748b" />
        ...
        <TouchableOpacity style={styles.button} onPress={handleSubmit} disabled={loading}>
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Record Expense</Text>}
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
