...
import { ChevronLeft, Home, Building2, Hotel, TrendingUp, Camera, MapPin } from "lucide-react-native";

const STAY_TYPES = [
  { id: "short_stay", label: "Short Stay", icon: Home },
...
export default function ListStayScreen() {
  ...
  const { createListing } = useStay();
  const [title, setTitle] = useState("");
  const [listingType, setListingType] = useState("short_stay");
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      router.replace("/(os)/stay/(tabs)/host");
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>List Your Stay</Text>
        <View style={{ width: 24 }} />
      </View>
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.typeRow}>
            {STAY_TYPES.map((t) => (
              <TouchableOpacity key={t.id} style={[styles.typeChip, listingType === t.id && styles.typeChipActive]} onPress={() => setListingType(t.id)}>
                <t.icon size={18} color={listingType === t.id ? "#fff" : "#1a5c4b"} />
                <Text style={[styles.typeChipText, listingType === t.id && styles.typeChipTextActive]}>{t.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.inputRow}><MapPin size={18} color="#9ca3af" /><TextInput style={[styles.input, { flex: 1, marginTop: 0 }]} placeholder="e.g. Nairobi" value={city} onChangeText={setCity} /></View>
        </View>
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Price per Night (KES) *</Text>
          <TextInput style={styles.input} placeholder="e.g. 5000" keyboardType="numeric" value={price} onChangeText={setPrice} />
        </View>
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Bedrooms</Text>
          <TextInput style={styles.input} placeholder="1" keyboardType="number-pad" value={bedrooms} onChangeText={setBedrooms} />
        </View>
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Bathrooms</Text>
          <TextInput style={styles.input} placeholder="1" keyboardType="number-pad" value={bathrooms} onChangeText={setBathrooms} />
        </View>
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Description</Text>
          <TextInput style={[styles.input, { minHeight: 100, textAlignVertical: "top" }]} multiline numberOfLines={4} placeholder="Describe your stay..." value={description} onChangeText={setDescription} />
        </View>
        <View style={styles.section}>
      ...
      <View style={styles.bottomBar}>
        <TouchableOpacity style={[styles.submitBtn, (!title || !price || !city || submitting) && styles.submitBtnDisabled]} onPress={handleSubmit} disabled={!title || !price || !city || submitting}>
          <Text style={styles.submitBtnText}>{submitting ? "Listing..." : "List Stay"}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
