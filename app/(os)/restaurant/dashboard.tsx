// ============================================================================
// MTAA Restaurant Module — Dashboard Screen
// ============================================================================
...
import { useDashboardAutoRefresh, useTableStatus, useKds, useInventory } from '@/lib/restaurant/hooks';
import { useRestaurantManager } from '@/lib/restaurant/hooks';
  ...
  return (
    ...
    >
      ...
      <View style={styles.kpiRow}>
        <KpiCard
          title="Today's Sales"
          value={metrics ? `KSh ${metrics.total_sales?.toFixed(2) || '0.00'}` : '—'}
          subtitle={`${metrics?.total_orders || 0} orders`}
          color="#10B981"
        />
        <KpiCard
          title="Active Tables"
          value={`${statusCounts?.occupied || 0}/${(statusCounts?.occupied || 0) + (statusCounts?.available || 0)}`}
          subtitle={`${statusCounts?.available || 0} available`}
          color="#3B82F6"
        />
      </View>
      ...
      <View style={styles.kpiRow}>
        <KpiCard
          ...
          subtitle={`${delayedOrders} delayed`}
          color={delayedOrders > 0 ? "#EF4444" : "#F59E0B"}
        />
        <KpiCard
          title="Avg Ticket"
          value={metrics ? `KSh ${metrics.average_ticket?.toFixed(2) || '0.00'}` : '—'}
          subtitle="per order"
          color="#8B5CF6"
        />
      </View>
        ...
        <View style={styles.alertSection}>
          ...
          {lowStockAlerts?.slice(0, 3).map((item) => (
            <View key={item.id} style={styles.alertItem}>
              <Text style={styles.alertText}>{item.name} — {item.current_quantity} / {item.reorder_level} {item.unit}</Text>
            </View>
          ))}
        </View>
      )}
...
function ActionButton({ icon, label, route, router }: any) {
  return (
...
function SummaryRow({ label, value, color = '#1F2937' }: any) {
  return (
...
const styles = StyleSheet.create({
  ...
  kpiRow: { flexDirection: 'row', paddingHorizontal: 12, marginTop: 12, gap: 12 },
  kpiCard: {
    flex: 1,
  ...
  kpiTitle: { fontSize: 12, color: '#6B7280', textTransform: 'uppercase', letterSpacing: 0.5 },
  kpiValue: { fontSize: 24, fontWeight: 'bold', marginTop: 4 },
  kpiSubtitle: { fontSize: 12, color: '#9CA3AF', marginTop: 2 },
  alertSection: {
    margin: 12,
  ...
  alertTitle: { fontSize: 14, fontWeight: '600', color: '#92400E', marginBottom: 8 },
  alertItem: { paddingVertical: 4 },
  alertText: { fontSize: 13, color: '#78350F' },
  sectionTitle: {
    fontSize: 16,
    fontWeight: '600',
    color: '#1F2937',
  ...
  actionLabel: { fontSize: 12, color: '#4B5563', marginTop: 8, textAlign: 'center' },
  summaryCard: {
    margin: 12,
    backgroundColor: '#FFFFFF',
    borderRadius: 12,
