...
export default function SendScreen() {
  ...
  const { balance, send, loading } = useWalletStore();
  ...
  const valid = address.length > 0 && parsedAmount > 0 && parsedAmount <= balance;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      Alert.alert("Success", `Sent $${parsedAmount.toFixed(2)}`);
      router.back();
    } catch (err: any) {
      Alert.alert("Send Failed", err.message || "Could not complete send");
    }
  };
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.confirmCard}>
          ...
          <Text style={[styles.confirmValue, { fontSize: 14 }]} numberOfLines={1}>
            {address}
          </Text>
    ...
    >
      ...
      <ScrollView style={styles.form}>
        ...
        <TouchableOpacity
          ...
          disabled={!valid}
        >
          <Text style={styles.buttonText}>Review Send</Text>
        </TouchableOpacity>
      </ScrollView>
    </KeyboardAvoidingView>
  );
