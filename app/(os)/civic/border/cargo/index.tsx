...
const cargoModules = [
  { route: '/(os)/civic/border/cargo/manifests', label: 'Manifests', icon: '📋', desc: 'Cargo manifests & declarations' },
  { route: '/(os)/civic/border/cargo/containers', label: 'Containers', icon: '📦', desc: 'Container tracking & status' },
];

export default function CargoHub() {
  const router = useRouter();
  return (
    <SafeAreaWrapper>
      <ScreenHeader title="Cargo & Manifests" subtitle="Border cargo management" />
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        {cargoModules.map(mod => (
