...
export default function PropertyPaymentScreen() {
  ...
  const { getPropertyById, currentBooking } = useProperty();
  ...
  const amount = booking?.total_amount || property?.price || 0;
  const canPay = account && account.available_balance >= amount;

  async function handlePay() {
    if (!canPay) {
      Alert.alert("Insufficient Balance", `You need KES ${amount.toLocaleString()} but have KES ${(account?.available_balance || 0).toLocaleString()}.`);
      return;
    }
    if (!booking?.id) {
    ...
    setPaying(true);
    try {
      // Use wallet transfer to pay the property host
      ...
      if (result.success) {
        Alert.alert("Payment Successful", `KES ${amount.toLocaleString()} paid for ${property?.title}.`);
        router.replace("/(os)/property/(tabs)/bookings");
      } else {
        Alert.alert("Payment Failed", result.error || "Transaction could not be completed.");
      }
    } catch (err: any) {
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        >
          <View style={styles.methodIcon}>
            <Wallet size={22} color={method === "wallet" ? "#1a5c4b" : "#6b7280"} />
          </View>
          <View style={styles.methodInfo}>
            <Text style={styles.methodName}>MTAA Wallet</Text>
            <Text style={styles.methodDesc}>
              Balance: KES {(account?.available_balance || 0).toLocaleString()}
            </Text>
          </View>
        ...
        <TouchableOpacity
          style={[styles.methodCard, method === "mpesa" && styles.methodCardActive]}
          onPress={() => setMethod("mpesa")}
        >
          <View style={styles.methodIcon}>
            <CreditCard size={22} color={method === "mpesa" ? "#1a5c4b" : "#6b7280"} />
          </View>
          <View style={styles.methodInfo}>
            <Text style={styles.methodName}>M-Pesa</Text>
            <Text style={styles.methodDesc}>Pay via mobile money</Text>
          </View>
          <View style={[styles.radio, method === "mpesa" && styles.radioActive]}>
        ...
        >
          <View style={styles.methodIcon}>
            <CreditCard size={22} color={method === "card" ? "#1a5c4b" : "#6b7280"} />
          </View>
          <View style={styles.methodInfo}>
        ...
        <View style={styles.securityNote}>
          ...
          <Text style={styles.securityText}>
            All payments are secured and encrypted. Your financial data is never stored on this device.
          </Text>
        </View>
      </ScrollView>
      ...
      <View style={styles.bottomBar}>
        ...
        >
          <Text style={styles.payBtnText}>
            {paying ? "Processing..." : canPay ? `Pay KES ${amount.toLocaleString()}` : "Insufficient Balance"}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
