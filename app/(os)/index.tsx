...
const DEFAULT_CORE_APPS = [
  ...
  { id: "command", label: "Command", icon: "Shield", route: "/(os)/command/asis-simulator", color: "#10b981" },
];
...
const DEFAULT_DOMAIN_APPS = [
  ...
  { id: "studio", label: "Studio", icon: "Film", route: "/(os)/studio/dashboard", color: "#A78BFA" },
  { id: "voting", label: "Voting", icon: "Vote", route: "/(os)/voting", color: "#8B5CF6" },
...
export default function HomeScreen() {
  ...
  const loadLayout = async () => {
    try {
      ...
      if (saved) {
        ...
        if (layout.system?.length) setSystemApps(layout.system.filter((v,i,a)=>a.findIndex(t=>t.id===v.id)===i));
        if (layout.core?.length) setCoreApps(layout.core.filter((v,i,a)=>a.findIndex(t=>t.id===v.id)===i));
        if (layout.domain?.length) setDomainApps(layout.domain.filter((v,i,a)=>a.findIndex(t=>t.id===v.id)===i));
      }
    } catch (e) {
  ...
  // FIXED: No "Warrior" fallback — uses user account name or empty string
  const displayName = profile?.display_name || user?.email?.split("@")[0] || (user as any)?.user_metadata?.full_name || (user as any)?.user_metadata?.name || "User";
