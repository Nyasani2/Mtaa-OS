  ...
  const toggleBiometric = async () => {
    ...
    if (!isEnabled) {
      ...
      if (result.success) {
        ...
        setEnrolledAnywhere(true);
        await loadStatus();
        Alert.alert('Enabled', `${biometricType} login is now enabled on this device.`);
      } else {
        Alert.alert('Failed', result.error || 'Could not enable biometric');
      }
    } else {
      // Disabling — ask if just this device or all devices
      Alert.alert(
        'Disable Biometric',
        'Remove from this device only, or all devices?',
        [
          { text: 'This Device Only', onPress: async () => {
            const result = await removeThisDevice(user.id);
            if (result.success) {
              setIsEnabled(false);
              setThisDeviceEnrolled(false);
              await loadStatus();
            }
          }},
          { text: 'All Devices', style: 'destructive', onPress: async () => {
            const result = await revokeAllBiometric(user.id);
            if (result.success) {
              setIsEnabled(false);
              setEnrolledAnywhere(false);
              setThisDeviceEnrolled(false);
              setDevices([]);
              await loadStatus();
              Alert.alert('Revoked', 'Biometric login removed from all devices.');
            }
          }},
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.statusCard}>
        <View style={styles.statusRow}>
          ...
          <View style={styles.statusTextCol}>
            ...
            <Text style={[styles.statusValue, { color: enrolledAnywhere ? '#10b981' : '#64748b' }]}>
              {enrolledAnywhere ? 'Active on ' + devices.length + ' device(s)' : 'Not enrolled'}
            </Text>
          </View>
        </View>
      </View>
        ...
        <View style={styles.row}>
          ...
          <View style={styles.textCol}>
            ...
            <Text style={styles.sublabel}>
              {isAvailable
                ? thisDeviceEnrolled
                  ? `Use ${biometricType} to unlock on this device`
                  : `Enable ${biometricType} for this device`
      ...
      {thisDeviceEnrolled && (
        <TouchableOpacity style={styles.testBtn} onPress={testBiometric}>
          ...
          <Text style={styles.testText}>Test Biometric</Text>
        </TouchableOpacity>
      )}
        ...
        <View style={styles.section}>
          ...
          {devices.map(device => (
            <View key={device.deviceId} style={styles.deviceRow}>
              <Ionicons
                name={device.platform === 'ios' ? 'phone-portrait' : 'phone-portrait-outline'}
                size={18}
                color="#64748b"
              />
      ...
      <View style={styles.infoCard}>
        ...
        <Text style={styles.infoText}>
          Your biometric data never leaves your device. MTAA only stores a device
          enrollment record so you can manage access across all your devices.
        </Text>
      </View>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  statusCard: { backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: 16, padding: 16, marginHorizontal: 16, marginTop: 16, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' },
  statusRow: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  statusTextCol: { flex: 1 },
  statusLabel: { fontSize: 13, color: '#94a3b8' },
  statusValue: { fontSize: 15, fontWeight: '600', marginTop: 2 },
  ...
  testBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginHorizontal: 16, marginTop: 16, padding: 14, backgroundColor: 'rgba(99,102,241,0.1)', borderRadius: 12, borderWidth: 1, borderColor: 'rgba(99,102,241,0.3)' },
  testText: { color: '#6366f1', fontWeight: '600', fontSize: 15 },
  section: { marginTop: 24, paddingHorizontal: 16 },
  sectionTitle: { fontSize: 13, fontWeight: '700', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 12 },
  deviceRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 14, marginBottom: 8, gap: 12 },
  deviceInfo: { flex: 1 },
  deviceName: { fontSize: 14, fontWeight: '500', color: '#e2e8f0' },
  deviceMeta: { fontSize: 12, color: '#64748b', marginTop: 2 },
  thisDeviceBadge: { backgroundColor: 'rgba(16,185,129,0.15)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 6 },
  thisDeviceText: { fontSize: 11, color: '#10b981', fontWeight: '600' },
  infoCard: { flexDirection: 'row', alignItems: 'flex-start', backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 16, marginHorizontal: 16, marginTop: 16, marginBottom: 40, gap: 12 },
