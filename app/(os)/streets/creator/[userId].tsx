...
import { getUserPosts, getUserProfile, getFollowerCounts } from '@/lib/services/streets-service';
import type { StreetPost } from '@/lib/services/streets-service';
...
export default function CreatorProfileScreen() {
    ...
    try {
      ...
      if (append) {
        setPosts((prev) => {
          ...
          const newPosts = safeData.filter((p) => !existingIds.has(p.id));
  ...
  const renderItem = useCallback(({ item }: { item: StreetPost }) => {
    ...
    const isText = !item.media_url || item.media_type === 'text';

    // ACTUAL post content — no placeholders
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.center}>
          ...
          <TouchableOpacity style={styles.retryBtn} onPress={() => loadData(1)}>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.profileSection}>
        ...
        <Text style={styles.profileName}>
          {displayName}
          {isVerified && <Text style={{ color: '#3897f0' }}> ✓</Text>}
        </Text>
        {profile?.username && (
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.stat}>
            <Text style={styles.statNumber}>{followerCounts.followers}</Text>
            <Text style={styles.statLabel}>Followers</Text>
          </View>
          <View style={styles.stat}>
            <Text style={styles.statNumber}>{followerCounts.following}</Text>
        ...
        {(['all', 'pics', 'videos'] as TabType[]).map((t) => (
          ...
          >
            <Text style={[styles.tabText, tab === t && styles.tabTextActive]}>
              {t === 'all' ? 'All' : t === 'pics' ? 'Photos' : 'Videos'}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      {filtered.length === 0 ? (
        <View style={styles.emptyGrid}>
          ...
          <Text style={styles.emptyText}>
            {tab === 'all' ? 'No posts yet' : `No ${tab} posts yet`}
          </Text>
        </View>
      ) : (
        <FlatList
...
const styles = StyleSheet.create({
  ...
  avatarFallback: { backgroundColor: '#E91E63', alignItems: 'center', justifyContent: 'center' },
  profileAvatarText: { color: '#fff', fontSize: 32, fontWeight: '700' },
  profileName: { color: '#fff', fontSize: 18, fontWeight: '600', marginTop: 12 },
  profileHandle: { color: '#666', fontSize: 14, marginTop: 4 },
  profileBio: { color: '#888', fontSize: 13, marginTop: 8, textAlign: 'center', paddingHorizontal: 32 },
  ...
  gridVideo: { width: '100%', height: '100%', position: 'relative' },
  ...
  gridText: { width: '100%', height: '100%', padding: 8, justifyContent: 'center', backgroundColor: '#1a1a1a' },
  gridTextContent: { color: '#fff', fontSize: 11, lineHeight: 15 },

  emptyGrid: { flex: 1, alignItems: 'center', justifyContent: 'center' },
