...
export default function ProfileScreen() {
  ...
  const loadPosts = async () => {
    let query = supabase.from('streets_posts').select('*').eq('user_id', targetId);
    ...
    if (activeTab === 'videos') {
      ...
      query = supabase.from('streets_posts').select('*').in('id', contentIds);
  ...
  const handleFollow = async () => {
    ...
    try {
      if (isFollowing) {
        await supabase.from('streets_follows').delete()
