...
export default function DriverOnboarding() {
  ...
  const submit = async () => {
    ...
    if (Object.keys(photos).length < 5) { setErr('All 5 vehicle photos are required.'); return; }
    if (CONDITION.some(c => !cond[c])) { setErr('Tick every condition item.'); return; }
    if (!docs.insurance || !docs.conduct) { setErr('Insurance + Good Conduct docs are required.'); return; }
    setBusy(true);
    try {
      // 1) PIN / biometric confirmation FIRST
      const auth = await requestPaymentAuth(fee);
      if (!auth) { setErr('Cancelled: PIN or biometric required.'); setBusy(false); return; }
      // 2) wallet balance gate
      ...
      if (bal < fee) { setErr('❌ Insufficient balance (KES ' + bal + '). Top up KES ' + fee + '.'); setBusy(false); return; }
      // 3) payee = garage owner (50% share) or self
      let payee = user!.id;
      if (garageId) {
        const { data: g } = await supabase.from('mtaxi_garages').select('owner_id').eq('id', garageId).maybeSingle();
        if (g?.owner_id) payee = g.owner_id;
      }
      // 4) atomic debit with PIN/biometric proof
      const { error: se } = await supabase.rpc('mtaa_settle', {
        p_payer_id: user!.id, p_payee_id: payee, p_total: fee,
        p_platform_rate_pct: 50, p_wht_rate_pct: 0,
        p_reference: 'driver-onboard-' + Date.now(),
        p_tx_pin: auth?.pin || null, p_biometric_ok: !!auth?.biometric,
      });
      ...
      window.alert('✅ KES ' + fee + ' paid. You are now a ' + vType.toUpperCase() + ' driver.');
      router.back();
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: '#0f0f1a', padding: 16 }} contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      <Text style={{ color: '#8892b0', marginBottom: 8 }}>{garageId ? 'Activate under your garage' : 'Activate to take rides'} · Fee KES {fee.toLocaleString()}</Text>
      ...
      <Text style={lbl}>Vehicle year (≤20 yrs)</Text><TextInput style={inp} value={f.vehicle_year} onChangeText={set('vehicle_year')} placeholderTextColor="#555" keyboardType="numeric" />
      ...
      <Text style={lbl}>Licence expiry</Text><TextInput style={inp} value={f.license_expiry} onChangeText={set('license_expiry')} placeholderTextColor="#555" />
      <Text style={{ ...lbl, color: '#fff', fontSize: 14 }}>📷 Vehicle photos — all 5 required *</Text>
      ...
      <Text style={{ ...lbl, color: '#fff', fontSize: 14 }}>✅ Condition — tick all *</Text>
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
        {CONDITION.map(c => (
      ...
      <Text style={{ ...lbl, color: '#fff', fontSize: 14 }}>📄 Documents *</Text>
      ...
      <TouchableOpacity onPress={submit} disabled={busy} style={{ backgroundColor: '#2ecc71', borderRadius: 12, paddingVertical: 14, alignItems: 'center', opacity: busy ? 0.6 : 1 }}>
