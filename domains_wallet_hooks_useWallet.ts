...
export function useWalletReceive() {
  ...
  const receive = useCallback(async (amount: number, fromUserId?: string, description?: string) => {
    if (!user?.id) return { success: false, error: 'Not authenticated' };
    setReceiving(true);
    setError(null);
    try {
      // Credit wallet
      const { error: err } = await supabase.rpc('mtaa_credit_wallet', {
        p_user_id: user.id,
        p_amount: amount,
        p_description: description || 'Wallet credit',
        p_reference: fromUserId || null,
        p_topup_method: 'transfer',
      });
      if (err) throw err;
      const { data: tx } = await supabase
        .from('wallet_transactions')
  ...
  return { receive, receiving, error, lastTx };
}
