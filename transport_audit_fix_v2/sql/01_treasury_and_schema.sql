-- ============================================================
-- MTAA TRANSPORT TREASURY + SCHEMA FIX
...
ADD COLUMN IF NOT EXISTS mtaa_fee_percent numeric DEFAULT 15,
ADD COLUMN IF NOT EXISTS gov_tax_percent numeric DEFAULT 16;
...
-- 5. Treasury routing function (schema-aligned)
...
BEGIN
  ...
  WHERE user_id = v_ride.passenger_id AND currency = 'KES' LIMIT 1;
  ...
  v_mtaa_fee := ROUND(v_total * (COALESCE(v_ride.mtaa_fee_percent, 15) / 100), 2);
  v_gov_tax := ROUND(v_total * (COALESCE(v_ride.gov_tax_percent, 16) / 100), 2);
  ...
  IF v_mtaa_wallet IS NOT NULL THEN
    ...
    INSERT INTO wallet_transactions (
      ...
      'MTAA platform fee - Ride ' || ride_id,
      ride_id, 'mtaa_fee', jsonb_build_object('direction', 'in')
    );
  END IF;

  -- 3. Credit Government treasury
  IF v_gov_wallet IS NOT NULL THEN
    ...
    INSERT INTO wallet_transactions (
      ...
      'Gov tax (VAT) - Ride ' || ride_id,
      ride_id, 'gov_tax', jsonb_build_object('direction', 'in')
    );
  END IF;
  ...
  IF v_driver_wallet IS NOT NULL AND v_ride.driver_id IS NOT NULL THEN
    ...
    INSERT INTO wallet_transactions (
      ...
      'Trip earnings',
      ride_id, 'driver_earnings', jsonb_build_object('direction', 'in')
    );
  END IF;
