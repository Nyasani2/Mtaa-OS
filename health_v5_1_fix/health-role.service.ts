...
export const ROLE_DASHBOARD_ROUTES: Record<string, string[]> = {
  system_admin: [
    "/(os)/health/system/analytics",
    "/(os)/health/system/roles",
    "/(os)/health/system/integrations",
    "/(os)/health/government/verify-facilities",
  ],
  facility_admin: [
    "/(os)/health/hospital-admin/revenue",
    "/(os)/health/hr/payroll",
    "/(os)/health/hr/attendance",
    "/(os)/health/hr/shifts",
    "/(os)/health/hr/leave",
  ],
  hospital_admin: [
  ...
  doctor: [
    "/(os)/health/doctor/queue",
    "/(os)/health/doctor/schedule",
    "/(os)/health/doctor/prescribe",
    "/(os)/health/doctor/lab-orders",
    "/(os)/health/doctor/earnings",
  ],
  nurse: [
    "/(os)/health/nurse/beds",
    "/(os)/health/nurse/medication",
    "/(os)/health/nurse/handover",
  ],
  pharmacist: [
    "/(os)/health/pharmacy/pos",
    "/(os)/health/pharmacy/inventory",
    "/(os)/health/pharmacy/dispense",
    "/(os)/health/pharmacy/interactions",
    "/(os)/health/pharmacy/suppliers",
  ],
  lab_technician: [
    "/(os)/health/lab/queue",
    "/(os)/health/lab/critical",
    "/(os)/health/lab/equipment",
  ],
  radiologist: [
    "/(os)/health/lab/queue",
    "/(os)/health/lab/equipment",
  ...
  receptionist: [
    "/(os)/health/receptionist/register",
    "/(os)/health/receptionist/checkin",
    "/(os)/health/receptionist/queue",
  ],
  cashier: [
    "/(os)/health/cashier/payments",
    "/(os)/health/cashier/insurance",
    "/(os)/health/cashier/invoices",
    "/(os)/health/cashier/revenue",
  ],
  accountant: [
    "/(os)/health/accountant/revenue",
    "/(os)/health/accountant/budget",
    "/(os)/health/accountant/procurement",
    "/(os)/health/accountant/tax",
    "/(os)/health/accountant/compliance",
  ],
  hr_manager: [
    "/(os)/health/hr/payroll",
    "/(os)/health/hr/attendance",
  ...
  ambulance_dispatcher: [
    "/(os)/health/ambulance/dispatches",
    "/(os)/health/ambulance/location",
    "/(os)/health/ambulance/log",
  ],
  emergency_responder: [
    "/(os)/health/ambulance/dispatches",
    "/(os)/health/emergency",
...
class HealthRoleService {
  async getPrimaryStaffRecord(userId: string): Promise<StaffRecord | null> {
    try {
      ...
      if (error) {
        // PGRST116 = no rows returned — this is OK, user is a patient
  ...
  isAdmin(role: HealthRole | string): boolean {
    return ["system_admin", "facility_admin", "hospital_admin", "government_admin"].includes(
      role as string
    );
  }

  isClinical(role: HealthRole | string): boolean {
    return ["doctor", "nurse", "pharmacist", "lab_technician", "radiologist"].includes(
      role as string
    );
  }
