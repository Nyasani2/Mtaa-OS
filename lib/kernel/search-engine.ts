...
export class SearchEngine {
  ...
  async search<T = any>(
    ...
    if (!config) {
      throw new Error(`Unknown search config: ${configKey}`);
    }
  ...
  async globalSearch(
    q: string,
    limitPerTable: number = 5
  ): Promise<Record<string, SearchResult>> {
    const results: Record<string, SearchResult> = {};

    const promises = Object.keys(SEARCH_CONFIGS).map(async key => {
      results[key] = await this.search(key, {
        q,
        perPage: limitPerTable,
      });
    });
  ...
  async createIndex(
    table: string,
    column: string,
    type: 'text' | 'trgm' | 'vector' = 'text'
  ): Promise<boolean> {
    const { error } = await this.client.rpc(
      'mtaa_create_search_index',
      {
        p_table: table,
        p_column: column,
        p_type: type,
  ...
  async getSearchStats(): Promise<{
    ...
    const { data: avg } = await this.client.rpc(
      'mtaa_search_avg_results'
    );

    const { data: top } = await this.client.rpc(
      'mtaa_search_top_queries',
      {
        p_limit: 10,
      }
    );

    const { data: zero } = await this.client
      .from('search_logs')
      .select('query')
      .eq('results_count', 0)
      .limit(20);
