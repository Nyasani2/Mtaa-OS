...
export class PrisonFacilitiesService {
  ...
  static async createFacility(data: Partial<PrisonFacility>): Promise<PrisonFacility> {
    const { data: result, error } = await supabase.from('prison_facilities').insert(data).select().single();
    if (error) throw error;
    return result;
  }

  static async updateFacility(id: string, data: Partial<PrisonFacility>): Promise<PrisonFacility> {
    const { data: result, error } = await supabase.from('prison_facilities').update(data).eq('id', id).select().single();
    if (error) throw error;
    return result;
  }
}
