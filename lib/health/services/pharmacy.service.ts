...
export const pharmacyService = {
  ...
  async processSale(saleData: any, items: any[]) {
    const { data: sale, error: saleError } = await supabase.from('health_pos_transactions').insert({ ...saleData, status: 'completed', created_at: new Date().toISOString() }).select().maybeSingle();
  ...
  async checkInteraction(drugA: string, drugB: string) {
    const { data } = await supabase.from('health_pharmacy_inventory').select('*').or(`and(drug_a.ilike.${drugA},drug_b.ilike.${drugB}),and(drug_a.ilike.${drugB},drug_b.ilike.${drugA})`).maybeSingle();
    if (data) return data;
