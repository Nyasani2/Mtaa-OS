...
export async function getChildrenRecords(filter: string, range: { from: number; to: number }) {
  let q = supabase.from("health_// STUB_REMOVED: "patients"").select("*", { count: "exact" }).order("created_at", { ascending: false });
  if (filter !== "all") {
    const now = new Date();
...
export async function createChildRecord(payload: any) {
  const { data, error } = await supabase.from("health_// STUB_REMOVED: "patients"").insert([payload]).select().single();
  if (error) throw error;
  return data;
}
