...
export class PatientService {
  static async getPatientByUserId(userId: string): Promise<HealthPatient | null> {
    const { data, error } = await supabase.from("health_patients").select("*").eq("user_id", userId).maybeSingle();
    if (error) return null; return data;
  }
  static async createPatient(patientData: Partial<HealthPatient>): Promise<HealthPatient> {
    const { data, error } = await supabase.from("health_patients").insert(patientData).select().maybeSingle();
  ...
  static async updateProfile(userId: string, data: Partial<HealthPatient>): Promise<HealthPatient> {
    const { data: result, error } = await supabase.from("health_patients").update(data).eq("user_id", userId).select().maybeSingle();
