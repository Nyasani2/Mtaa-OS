...
export function useAmbulanceDispatches(filter: string) {
  return usePaginatedQuery(["ambulance-dispatches", filter], (range) => getAmbulanceDispatches(filter, range));
}
...
export function useHandoverDispatch() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: handoverDispatch,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["ambulance-dispatches"] }); qc.invalidateQueries({ queryKey: ["dispatch-detail"] }); },
  });
}
