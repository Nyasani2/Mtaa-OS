...
export function useHospitalAccounting(facilityId: string | null, period: string) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const cashRevenue = payments?.filter(p => p.payment_method === 'cash').reduce((s, p) => s + (p.amount || 0), 0) || 0;
      const cardRevenue = payments?.filter(p => p.payment_method === 'card').reduce((s, p) => s + (p.amount || 0), 0) || 0;
      const walletRevenue = payments?.filter(p => p.payment_method === 'wallet').reduce((s, p) => s + (p.amount || 0), 0) || 0;
      const mpesaRevenue = payments?.filter(p => p.payment_method === 'mpesa').reduce((s, p) => s + (p.amount || 0), 0) || 0;
      const insuranceRevenue = payments?.filter(p => p.payment_method === 'insurance').reduce((s, p) => s + (p.amount || 0), 0) || 0;
      const outstanding = invoices?.filter(i => i.status === 'unpaid' || i.status === 'partial').reduce((s, i) => s + (i.balance_due || 0), 0) || 0;
      const outstandingCount = invoices?.filter(i => i.status === 'unpaid' || i.status === 'partial').length || 0;
